...
void JsonArray_add(struct SeaJsonArray* self, struct SeaJsonValue* value, struct SeaAllocator* allocator);
...
struct SeaJsonValue* CreateBool(const bool val, struct SeaAllocator* alloc) {
	...
	struct SeaJsonValue* value = SeaAllocator.alloc(alloc, sizeof(struct SeaJsonValue));
	if (!value) return NULL;
	value->type = SEA_JSON_BOOL;
	value->boolean = val;
	value->timestamp = SeaTime.getMillis();
...
struct SeaJsonValue* CreateString(const char* val, struct SeaAllocator* alloc) {
	...
	uint8_t* buffer =  SeaAllocator.alloc(alloc, bufferLen);
...
struct SeaJsonValue* CreateArray(struct SeaAllocator* alloc) {
	...
	uint8_t *buffer = SeaAllocator.alloc(
		alloc,
...
static struct SeaJsonValue* ParseString(InternalJsonParser* parser) {
	...
	char *str = SeaAllocator.alloc(parser->allocator, len + 1);
	...
	SeaAllocator.free(parser->allocator, str);
...
static struct SeaJsonValue* ParseNumber(InternalJsonParser* parser) {
	...
	const size_t len = parser->pos - start;
	char* num_str = SeaAllocator.alloc(parser->allocator, len + 1);
	if (!num_str) return NULL;
	strncpy(num_str, parser->json + start, len);
	num_str[len] = '\0';

	const double num = atof(num_str);
	SeaAllocator.free(parser->allocator, num_str);
...
static char* JsonValue_toString(const struct SeaJsonValue* self, struct SeaAllocator* allocator) {
	if (!self || !allocator || !allocator->alloc) return NULL;

	struct SeaStringBuffer buffer = {
		.data = (char*)SeaAllocator.alloc(allocator, 256),
		.size = 256,
		.pos = 0,
		.allocator = allocator
	};
...
void FreeArray(struct SeaJsonValue * self, struct SeaAllocator * alloc) {
	...
	SeaAllocator.free(alloc, self->array);
	SeaAllocator.free(alloc, self);
}

void FreeObject(const struct SeaJsonValue * self, struct SeaAllocator * alloc) {
	...
	for (size_t i = 0; i < self->object->bucketCount; i++) {
		...
		for (const InternalJsonObjectEntry* e = next; e != NULL; e = next) {
			...
			SeaAllocator.free(alloc, e->key);
		}
	}
	SeaAllocator.free(alloc, self->object);
}
...
enum SeaErrorType JsonObject_put(struct SeaJsonObject* self, const char* key, struct SeaJsonValue* value, struct SeaAllocator* alloc) {
	...
	InternalJsonObjectEntry* e = SeaAllocator.alloc(alloc, sizeof(*e));
	e->key_len = strlen(key);
	e->key = SeaAllocator.strdup(alloc, key, e->key_len);
	e->value = value;
	e->next  = *bucket;
	*bucket = e;
	self->size++;
...
const char** JsonObject_keys(const struct SeaJsonObject *self, const struct SeaAllocator *alloc) {
	if (!self || !alloc) return NULL;
	const size_t count = self->size;
	const char **keys = SeaAllocator.allocEx(alloc, count * sizeof(char *), SEA_ALIGNOF(char *));
	if (!keys) return NULL;
	size_t idx = 0;
	for (size_t i = 0; i < self->bucketCount; i++) {
		for (const InternalJsonObjectEntry *e = self->buckets[i]; e != NULL; e = e->next) {
			keys[idx++] = e->key;
		}
	}
	return keys;
...
void JsonArray_add(struct SeaJsonArray* self, struct SeaJsonValue* value, struct SeaAllocator* allocator) {
	...
	if (self->count >= self->capacity) {
		...
		struct SeaJsonValue** new_items = SeaAllocator.alloc(allocator, new_size);
		if (!new_items) return;
		...
		if (self->items) {
			...
			SeaAllocator.free(allocator, self->items);
		}
