...
static SEA_Allocator HEAP_ALLOCATOR = {
	.context = NULL,
...
SEA_Allocator* const SEA_Allocator_Heap = &HEAP_ALLOCATOR;
#if SEA_CONFIG_ENABLE_PREFIXLESS == 1
//SEA_Allocator* const Allocator_Heap = &HEAP_ALLOCATOR;
#endif
...
char* SEA_Allocator_strdup(SEA_Allocator* self, const char* str, const size_t len) {
...
void* SEA_Allocator_alloc(SEA_Allocator* self, const size_t size) {
	if (!self || !self->alloc || size == 0) return NULL;
	return self->alloc(self->context, size);
}

void* SEA_Allocator_allocAligned(SEA_Allocator* self, const size_t size, const size_t alignment) {
